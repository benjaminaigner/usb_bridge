...
#define CLEAR_HID_KEYBOARD_REPORT(x)   memset(x, 0, KEYBOARD_REPORT_SIZE);
#define MOUSE_REPORT_SIZE        5
#define CLEAR_HID_MOUSE_REPORT(x)   memset(x, 0, MOUSE_REPORT_SIZE);
#define JOYSTICK_REPORT_SIZE        13
#define CLEAR_HID_JOYSTICK_REPORT(x)   memset(x, 0, JOYSTICK_REPORT_SIZE);

/**
 * @brief	HID interface init routine.
 * @param	hUsb		: Handle to USB device stack
 * @param	pIntfDesc	: Pointer to HID interface descriptor -> determines all of the 3 HID types
 ...
 */
extern ErrorCode_t HID_Init(USBD_HANDLE_T hUsb,
							  USB_INTERFACE_DESCRIPTOR *pIntfDesc,
							  uint32_t *mem_base,
							  uint32_t *mem_size);

/**
 * @brief	HID tasks, processing mouse, keyboard and joystick
 * @return	On success returns LPC_OK.
 */
extern void HID_Tasks(void);
